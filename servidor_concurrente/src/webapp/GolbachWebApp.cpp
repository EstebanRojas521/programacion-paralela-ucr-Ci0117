...
void GolbachWebApp::start(){

}

void GolbachWebApp::stop(){

}


bool GolbachWebApp::handleHttpRequest(HttpRequest& httpRequest,
HttpResponse& httpResponse){
  ...
  // If the request starts with "fact/" is for this web app
    if (httpRequest.getURI().rfind("/fact", 0) == 0) {
        return this->serveGolbach(httpRequest, httpResponse);
    }
...
bool GolbachWebApp::serveHomepage(HttpRequest& httpRequest
  , HttpResponse& httpResponse) {
  (void)httpRequest;
...
bool GolbachWebApp::serveGolbach(HttpRequest& httpRequest
  , HttpResponse& httpResponse) {
  (void)httpRequest;
  ...
  if (std::regex_search(httpRequest.getURI(), matches, inQuery)) {
    ...
    // Build the body of the response
    std::string title = "Golbach of " + std::to_string(number);
    httpResponse.body() << "<!DOCTYPE html>\n"
      << "<html lang=\"en\">\n"
      << "  <meta charset=\"ascii\"/>\n"
      << "  <title>" << title << "</title>\n"
